...
print_oldest <- function(df, k=5) {
  topk <- df %>% mutate(age=2020-birthyear) %>% 
    arrange(desc(age)) %>% filter(type=='rep')
  for (i in 1:k) {
    cat(topk$age[i], topk$full_name[i], '\n')
  }
}
congress %>% print_oldest(5)
```
<br/>
...
print_oldest <- function(df, k=5) {
  ...
  topk <- df %>% mutate(age=2020-birthyear) %>% 
    filter(type=='rep') %>% arrange(desc(age))
  for (i in 1:k) {
    cat(topk$age[i], topk$full_name[i], '\n')
  }
}
