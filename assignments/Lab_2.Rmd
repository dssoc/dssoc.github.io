...
# your answer here
congress %>% 
  filter(gender=='F', party=='Democrat', type=='sen') %>% 
  arrange(birthyear) %>% 
  slice(1:1, n():n())
```
<br/>
...
congress %>% 
  ...
  filter(party=='Democrat', type=='sen', age > 60) %>% 
  nrow()
```
<br/>
...
# your answer here
congress %>% 
  mutate(age = 2022-birthyear, is_old = age > 55) %>% 
  count(gender, is_old)
```
<br/>
...
```{r}
# your answer here
travel_restrictions %>% intersect(require_masks)
```
<br/>
...
```{r}
# your answer here
travel_restrictions %>% setdiff(require_masks)
```
