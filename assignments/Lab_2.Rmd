...
**3. Identify the oldest and youngest female senator democrats using only R code.**
...
Note: you can approximate age using the formula `age = 2022-birthyear`.
```{r}
# your answer here
```
<br/>
...
**6. For this problem, use the `congress_contact` dataframe that includes the contact information for each member of congress. Using `gather`, create a new dataframe where each row corresponds to a valid twitter, facebook, or youtube social media account username, then compute the total number of valid accounts for each political party. Then accomplish the same task using `pivot_longer`.**
...
HINT: the account name will be an emptry string in cases where the member of congress does not have a valid user account, so you can filter using the conditional `column_name != ''`. This compares each value in the column called `column_name` to an empty string to determine whether it is a valid account. 
