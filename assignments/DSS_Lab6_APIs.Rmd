...
(4) `dnc_candidates_flw_el.Rdata`: an edgelist of the top five DNC cadidates for president (according to delegates won as of February 15, 2020), and the accounts they follow on Twitter.
...
#### **Exercise 2**: Follow the steps to set up your Twitter API credentials. Instructions for this can be found [here](https://cran.r-project.org/web/packages/rtweet/vignettes/auth.html). When you have completed the steps, you will enter your below key, secret key, token, and token secret below, but **BE SURE TO REMOVE THESE BEFORE SUBMITTING YOUR ASSIGNMENT**
```{r, eval = F}
...
#### **Exercise 3**: Using the `dnc_canidates_flw_el.Rdata` dataset, and the [`rtweet` package](https://cloud.r-project.org/web/packages/rtweet/rtweet.pdf), create a new dataset that contains additional meta information about all candidates and/or the accounts they follow. Once gathered, describe and demonstrate (e.g. in table or graph) some new insight (e.g. a summary statistic or pattern) that you gain from this additional data. What more does it tell you about the candidates, the accounts they follow, or how they relate to one another?
```{r}
# Your answer here
```
<br/>

#### **Exercise 4**: Follow [the steps](https://github.com/charlie86/spotifyr) to set up your Spotify API credentials. When you have finished, you will enter your client ID and secret below, but **BE SURE TO REMOVE THESE BEFORE SUBMITTING YOUR ASSIGNMENT.**
...
#### **Exercise 6**: Now, make a network visualization of the edge list created in Exercise 1. You may do this however you'd like. Although not necessary, you may consider reformatting your data so that the network visual contains only nodes for the Grammy nominees (i.e. those Spotify ids in the `artist_meta` dataset), with edges weighted by the number of related artists they share in common. You may also consider adding artist names from the `artist_meta` dataset as node attributes in your visualization. Alternatively, you may like to gather more information from the Spotify API and add it as node or edge information in your visualization (e.g. coloring nodes by primary genre that the artist belongs to). Be creative :)
