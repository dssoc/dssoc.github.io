...
Note: we don't need to get the actual lists of followers or Tweets - we just need the *number* of friends and tweets (i.e. **do not try to use `get_friends`**). You may need to refer to the [rtweet Package Documentation](https://cran.r-project.org/web/packages/rtweet/rtweet.pdf) to see which function to use.

Note: for various reasons, not all senators in our dataset have valid Twitter handles - we can ignore them for the purpose of this assignment.
