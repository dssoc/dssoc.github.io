...
senators %>% ggplot(aes(x=gender, y=statuses_count)) + geom_violin()
```
...
```{r}
# Your answer here
#msen <- senators %>% filter(gender=='M') %>% arrange(birthyear) %>% head(5)
#fsen <- senators %>% filter(gender=='F') %>% arrange(birthyear) %>% head(5)
...
#tweets <- get_timeline(mfsenators$twitter, n=10)
...
#idtweets <- lookup_tweets(senator_tweet_ids$tweet_ids)
