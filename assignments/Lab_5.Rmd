...
**3. Does the `dnc_candidates_flw_am` adjacency matrix contain one- or two-mode network information? What about the `dnc_candidates_flw_am` edgelist?**
```{r}
# Yor answer here or in text.
```
<br/>
...
**4. Using the `dnc_candidates_flw_am.Rdata` adjacency matrix (a data frame), create a graph (network) object. Make sure to include information about the edge weights (i.e. the number of accounts shared in common).**
```{r}
# Yor answer here or in text.
```
<br/>
...
**5. Describe three different metrics (e.g. betweenness centrality, degree, etc) that can be used to summarize networks or specific nodes/vertices within a network.**
```{r}
# Your answer here
```
<br/>
...
**7. Make a visual of your graph object, with edges scaled by weight (i.e. the number of accounts that the two candidates mutually follow). If you decide to make this graph in Gephi, feel free to submit a separate image of your final visual in addition to your .Rmd file.**
```{r}
# Yor answer here or in text.
```
<br/>
...
**8. Using the `dnc_candidates_flw_el` edgelist, calculate the degree for each candidate.**
```{r}
# Yor answer here or in text.
```
<br/>
...
**9. If you did not do so for Exercise 8, demonstrate how you would turn the `dnc_candidates_flw_el` edgelist into a graph object.**
```{r}
# Yor answer here or in text.
```
<br/>
...
**10. In last week's lab excerices, you were asked to indicate possible research topics that you would be interested in pursuing for your final project. Choose one of your topics and expand on it in more detail (i.e. at least one paragraph).**
```{r}
# your answer here
```
