...
NOTE: pay special attention to the `graph_from_data_frame` function. It will be our primary function for creating new graphs.
```
...
**5. Create a bipartite network representing links between members of congress and the subcommittees (not full committees) they are associated with using the `committee_memberships` dataframe and the `graph_from_data_frame` function. Keep this graph for future problems. You can verify that your network is bipartite using the `is_bipartite` function; show this output as your solution to this question - it should return TRUE.**
...
HINT: pay special attention to the `graph_from_data_frame` function arguments. Our use here may vary slightly from the example in the lab lecture or video.

HINT: see the section "Loading and configuring two-mode data" in the "Bipartite/Two-Mode Networks in igraph" required reading for information about how to make bipartite graphs in igraph. One way to create a bipartite graph in igraph is to use the line `V(g)$type <- bipartite_mapping(g)$type`, where `g` is the name of your graph. See from the documentation for `is_bipartite` that `is_bipartite` simply returns true if the `type` node attribute is set.
```{r}
# Your answer here.
```
<br/>
...
**11. In last week's lab exercise, you were asked to identify several possible datasets you could use for your final project. Now write two specific data science research questions and describe variables in that dataset that could allow you to answer the questions.**
