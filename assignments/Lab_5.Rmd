...
# get top 5 largest committees composed of senators
senate_comms <- committees %>% 
  inner_join(senators, by=c('bioguide'='bioguide_id')) %>% 
  filter(type=='sen')

comm_sel <- senate_comms %>% 
  count(committee) %>% 
  arrange(desc(n)) %>% 
  head(5)

largest_comms <- senate_comms %>% 
  inner_join(comm_sel, by='committee')

el <- largest_comms %>% select(committee, name)
...
tmp_el <- committees %>% select(committee, bioguide)
h <- graph_from_data_frame(tmp_el, directed=TRUE)
V(h)$type <- V(h)$name %in% congress$bioguide_id
is_bipartite(h)

A <- h %>% as_adjacency_matrix()
...
proj <- bipartite_projection(h)
g_comm = proj[[1]] # committee network

# add vertex attributes to politican network
ind <- as.factor(congress$bioguide_id)
g_pol = proj[[2]] %>% 
  set_vertex_attr("party", index = ind, congress$party) %>% 
  set_vertex_attr("gender", index = ind, congress$gender) %>% 
  set_vertex_attr("age", index = ind, 2020-congress$birthyear) %>% 
  set_vertex_attr("full_name", index = ind, congress$full_name)
```
<br/>
...
# Yor answer here or in text.
g_pol %>% 
  delete.edges(which(E(g_pol)$weight <= 5)) %>% 
  ggraph(layout='kk') + 
    geom_edge_link(width=0.5) +
    geom_node_point(aes(color=gender, size=age)) +
    geom_node_text(aes(label=full_name))
```
```
...
cent_df <- data.frame(
      deg=degree(g_pol), 
      wdeg=strength(g_pol),
      eig=eigen_centrality(g_pol)$vector,
      betw=betweenness(g_pol), 
      close=closeness(g_pol)
    )
cent_df$bioguide_id <- rownames(cent_df)
cent_df <- cent_df %>% inner_join(congress, by='bioguide_id')

cor(cent_df$wdeg, cent_df$eig)
cor(cent_df$wdeg, cent_df$betw)
cor(cent_df$wdeg, cent_df$close)
```
```
...
```{r}
# your answer here
cent_df %>% arrange(desc(wdeg)) %>% select(full_name) %>% head(3)
cent_df %>% arrange(desc(eig)) %>% select(full_name) %>% head(3)
```
```
written explanation here
```
...
cent_df %>% ggplot(aes(x=gender, y=wdeg)) + geom_violin()
cent_df %>% ggplot(aes(x=party, y=wdeg)) + geom_violin()
```
```
written explanation here
```
