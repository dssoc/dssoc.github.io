...
frequent_tokens %>% 
  head(20) %>% 
  ggplot(aes(y = reorder(word, n), x=n, fill = word)) +
...
frequent_tokens %>% 
  ...
  geom_bar(stat = "identity") +
    ...
    ylab("Frequency") +
    theme_minimal()
#substr(frequent_tokens$word,1,1)
...
# your answer here
senator_tweets %>% 
  filter(str_detect(text, 'forward')) %>% 
...
senator_tweets <- senator_tweets %>% 
  ...
  mutate(is_daytime=ifelse(hr<5 | hr>17, 'night', 'day'))

senator_tweets%>% 
  count(is_daytime) %>% 
  ggplot(aes(x=is_daytime, y=n)) + 
    geom_bar(stat='identity')
```
<br/>
...
# your answer here
senator_tweets %>% 
  select(status_id, text, hr, is_daytime) %>% 
  unnest_tokens(word, text) %>% 
  inner_join(get_sentiments("bing")) %>% 
  count(status_id, is_daytime, sentiment) %>% 
  group_by(is_daytime, sentiment) %>% 
  summarize(score=mean(n)) %>% 
  ggplot(aes(x=sentiment, y=score, fill=is_daytime)) +
