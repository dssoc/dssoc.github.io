...
**1. Using the `artist_meta` table, create a new factor variable (`is_pop`) that indicates whether the artist is associated with any pop genre (e.g. a genre that has the word "pop" in it). The labels for this factor variable should be 'pop' and 'not pop'.**
```{r}
...
**3. Change the theme of your scatterplot from Exercise 2, and apply appropriate x- and y-axis labels. Note: for theme information, see 28.6 in *R for Data Science* or 8.3 of *Data Visualization a Practical Introduction*.**
```{r}
artist_meta %>% ggplot(aes(followers, y=popularity)) + 
  geom_point() + 
...
**11. Provide three social science research questions that you would be interested in exploring for the group project. There are no right answers here. We just want to get a sense of interests.**
```{r}
# your answer here
```
