...
congress %>% 
  #mutate(social_media=as.numeric(!is.na(twitter))+as.numeric(!is.na(facebook))+as.numeric(!is.na(youtube))) %>% 
  mutate(social_media=ifelse(is.na(twitter),0,1) + ifelse(is.na(facebook),0,1) + ifelse(is.na(youtube),0,1)) %>% 
  select(party,social_media) %>% 
  #filter(party==c("Republican","Democrat")) %>% 
  #filter(party=='Democrat' | party=='Republican') %>% 
  group_by(party) %>% 
  summarise(total=sum(social_media))
```
```
...
tmp <- congress %>% 
  ...
  #inner_join(committees, by = c("full_name"="name")) %>% 
  group_by(bioguide_id) %>% 
...
inner_join(committees,congress, by = c("name"="full_name")) %>% 
  group_by(committee,party.y) %>% 
  count(name="amnt") %>% 
  group_by(party.y) %>% 
  summarise(total=mean(amnt))# %>% 
  #filter(party.y=="Democrat" | party.y == "Republican") #%>% 
  #ggplot() +
  #geom_bar(aes(party.y,total),stat="identity")+
  #labs(title = "Democrats on more committees on average",x="Party",y="Average committees")


combined <- inner_join(committees, congress, by = c("name" = "full_name"))

avg_committees <- combined %>% 
  filter(party.y == "Democrat" | party.y == "Republican") %>% 
  group_by(name) %>% 
  mutate(committee, total_committees = n())

avg_comm2 <- avg_committees %>% 
  group_by(party.y) %>% 
  summarise(avg_comts = mean(total_committees))
